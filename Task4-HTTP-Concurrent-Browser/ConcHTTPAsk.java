...
public class ConcHTTPAsk {
	...
	public static void main(String[] args) throws IOException {
		...
		ConcHTTPAsk server = new ConcHTTPAsk(serverPort);
		server.runServer();
	}

	private ConcHTTPAsk(int serverPort) throws IOException {
	...
	class Worker implements Runnable {
		...
		private void processRequest(BufferedReader inFromClient, DataOutputStream outFromServer) throws IOException {
			...
			String[] reqArray = clientRequest.split(" ");
			for (String s : reqArray)
				System.out.print(" " + s + " ");
			System.out.println("\n  Client has supplied server with the following URL: " + clientRequest);
			if (!clientRequest.startsWith("GET") || clientRequest.split(" ").length < 3) {
				outFromServer.writeBytes(httpResponse("400 Bad Request", "Something is wrong with the URL supplied by the client."));
			} else {
		...
		private Map<String, String> getHttpRequestParameters(String clientRequest, DataOutputStream outFromServer) throws IOException {
			...
			for (int i = 0; i < requestArray.length; i++) {
				if (requestArray[i].equals("hostname")) {
					...
					requestParameters.put(requestArray[i], requestArray[++i].replace('+', ' '));
				}
			}
			if (!requestArray[2].equals("ask") || requestParameters.get("port") == null || requestParameters.get("hostname") == null) {
				outFromServer.writeBytes(httpResponse("400 Bad Request", "Something is wrong with the URL supplied by the client."));
			} else {
